...
pub struct Settings {
    ...
    pub fancyzones_enabled: bool, // <-- Adăugat
    pub language: u32, 
}

impl Settings {
    pub fn load() -> Self {
        let mut snap = 1u32; let mut alttab = 1u32; let mut copilot = 1u32; let mut mem = 1u32; let mut fz = 0u32; let mut lang = 0u32;
        unsafe {
            ...
            if RegCreateKeyExW(HKEY_CURRENT_USER, w!("Software\\Gallery Inc\\IBBE-Hooker"), 0, PCWSTR::null(), REG_OPTION_NON_VOLATILE, KEY_READ, None, &mut hkey, None).is_ok() {
                ...
                let _ = RegQueryValueExW(hkey, w!("FeatureFancyZones"), None, None, Some(&mut fz as *mut _ as *mut u8), Some(&mut size)); size = 4u32;
                let _ = RegQueryValueExW(hkey, w!("Language"), None, None, Some(&mut lang as *mut _ as *mut u8), Some(&mut size));
                let _ = RegCloseKey(hkey);
            }
        }
        Self { snap_enabled: snap != 0, alttab_enabled: alttab != 0, copilot_enabled: copilot != 0, mem_cleaner_enabled: mem != 0, fancyzones_enabled: fz != 0, language: lang }
    }
    ...
    pub fn save(&self) {
        unsafe {
            ...
            if RegCreateKeyExW(HKEY_CURRENT_USER, w!("Software\\Gallery Inc\\IBBE-Hooker"), 0, PCWSTR::null(), REG_OPTION_NON_VOLATILE, KEY_WRITE, None, &mut hkey, None).is_ok() {
                ...
                let fz = if self.fancyzones_enabled { 1u32 } else { 0u32 };
                ...
                let _ = RegSetValueExW(hkey, w!("FeatureFancyZones"), 0, REG_DWORD, Some(std::slice::from_raw_parts(&fz as *const _ as *const u8, 4)));
